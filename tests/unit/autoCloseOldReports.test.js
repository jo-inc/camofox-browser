...
function triageIssue({ body = '', labels = [] }) {
  ...
  if (!dominated) {
    ...
    if (labels.includes('memory-leak')) {
      ...
      if (growthMb !== null && growthMb < 400) {
        return {
          action: 'close_below_threshold',
          growthMb,
          comments: [`Closing — native memory growth of ${growthMb}MB is below the 400MB alert threshold.`],
        };
      }
      ...
      const isIdle = (ctxMatch && parseInt(ctxMatch[1], 10) === 0 && (!tabMatch || parseInt(tabMatch[1], 10) === 0)) ||
                     (sessionsMatch && parseInt(sessionsMatch[1], 10) === 0 && (!tabMatch || parseInt(tabMatch[1], 10) === 0));
...
`;
  ...
  test('closes report if memory leak growth is below 400MB on modern version', () => {
    const body = `
## Environment
- **version:** 1.14.0
- **node:** v22.22.2
...
## Error
\`\`\`
Native memory grew by 250MB (baseline: 118MB, current: 368MB, high-water: 368MB)
\`\`\`
`;
    const result = triageIssue({ body, labels: ['auto-report', 'memory-leak'] });
    expect(result.action).toBe('close_below_threshold');
    expect(result.growthMb).toBe(250);
    expect(result.comments[0]).toContain('below the 400MB alert threshold');
  });
...
`;
  ...
  test('keeps genuine unbounded leak on modern version with active sessions open', () => {
    const body = `
## Environment
- **version:** 1.14.0
