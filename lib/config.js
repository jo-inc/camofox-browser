...
function inferProxyStrategy(proxyHost, explicitStrategy) {
  if (explicitStrategy) return explicitStrategy;
  if (proxyHost && proxyHost.endsWith('.decodo.com')) return 'backconnect';
  return 'round_robin';
}
...
function loadConfig() {
  return {
    ...
    proxy: {
      strategy: inferProxyStrategy(process.env.PROXY_HOST || '', process.env.PROXY_STRATEGY || ''),
      host: process.env.PROXY_HOST || '',
      port: process.env.PROXY_PORT || '',
      ports: parseProxyPorts(process.env.PROXY_PORTS, process.env.PROXY_PORT),
      username: process.env.PROXY_USERNAME || '',
      password: process.env.PROXY_PASSWORD || '',
      backconnectHost: process.env.PROXY_BACKCONNECT_HOST || 'gate.decodo.com',
