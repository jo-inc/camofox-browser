...
function loadConfig() {
  ...
  return {
    ...
    // Bind interface. Defaults to 127.0.0.1 (loopback-only) to keep browser-control
    // off non-local interfaces by default. Set CAMOFOX_HOST=0.0.0.0 only when the
    // service is intentionally exposed (e.g. Docker, Fly.io) AND CAMOFOX_ACCESS_KEY
    // is set to gate the routes. The launchd wrapper on macOS pins this to 127.0.0.1.
    host: (process.env.CAMOFOX_HOST || '127.0.0.1').trim(),
