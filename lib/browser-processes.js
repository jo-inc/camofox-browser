import fs from 'fs';
import { isWindowsBrowserProcess, killWindowsProcessTree, snapshotWindowsProcesses } from './windows-processes.js';
...
export function selectBrowserProcessVictims(entries, { myPid, excludePid = null } = {}) {
  return entries
    ...
    .filter((entry) => process.platform === 'win32' && entry.name != null
      ? isWindowsBrowserProcess(entry)
      : isBrowserProcessCmdline(entry.cmdline))
    .map(({ pid }) => pid);
}
