...
async function persistStorageState({ profileDir, userId, context, logger = console, nativeProfile = false }) {
  ...
  try {
    ...
    if (nativeProfile || context._persistentLease) {
      // Playwright's storageState()/cookies() requests can hang against the
      // Camoufox persistent Firefox context. The native profile owns cookie,
      // local-storage, and session-store persistence; lifecycle preferences
      // configured at launch make that durable without a protocol snapshot.
      await fs.writeFile(
        tmpMetaPath,
        JSON.stringify(
          {
      ...
      return { persisted: true, reason: 'native-profile', userDir, metaPath };
    }
