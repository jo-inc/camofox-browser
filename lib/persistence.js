...
async function quarantinePersistedStorageState({
  profileDir,
  userId,
  storageStatePath,
  reason = 'invalid',
  ...
  if (!profileDir && !storageStatePath) {
    return { quarantined: false, reason: 'disabled' };
  }

  const paths = storageStatePath
    ? { storageStatePath }
    : getUserPersistencePaths(profileDir, userId);
  const sourcePath = paths.storageStatePath;
  const quarantinePath = sourcePath.replace(/storage-state\.json$/, `storage-state.invalid-${Date.now()}.json`);

  try {
    await fs.rename(sourcePath, quarantinePath);
    logger?.warn?.('quarantined persisted storage state', {
      userId: String(userId),
      storageStatePath: sourcePath,
      quarantinePath,
      reason,
    });
    return { quarantined: true, storageStatePath: sourcePath, quarantinePath };
  } catch (err) {
    if (err?.code === 'ENOENT') return { quarantined: false, reason: 'missing' };
    logger?.warn?.('failed to quarantine persisted storage state', {
      userId: String(userId),
      storageStatePath: sourcePath,
      error: err?.message || String(err),
    });
    return { quarantined: false, reason: 'error', error: err };
  }
}
