import fs from 'fs';
import { isWindowsProcessCurrent, selectWindowsProcessTree, snapshotWindowsProcesses } from './windows-processes.js';

const BROWSER_PROCESS_RE = /camoufox(?:-bin)?(?:\.exe)?|firefox(?:-esr)?(?:\.exe)?|\/usr\/bin\/Xvfb\b/i;
...
export function snapshotOwnedBrowserProcesses(rootPid, procRoot = '/proc') {
  if (process.platform === 'win32' && procRoot === '/proc') {
    ...
    return selectWindowsProcessTree(rootPid, processes)
      .filter((proc) => BROWSER_PROCESS_RE.test(proc.name) || BROWSER_PROCESS_RE.test(proc.cmdline));
  }
