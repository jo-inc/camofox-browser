...
function redactProxyServer(server) {
  if (typeof server !== 'string') return '<invalid-proxy-server>';
  try {
    const parsed = new URL(server);
    if (!ALLOWED_PROXY_SCHEMES.has(parsed.protocol) || !parsed.hostname) {
      return '<invalid-proxy-server>';
    }
...
export function redactProxy(proxy) {
  if (!proxy) return null;
  return {
    server: redactProxyServer(proxy.server),
...
export function normalizeRequestProxy(proxy) {
  ...
  if ((parsed.pathname && parsed.pathname !== '/') || server.includes('?') || server.includes('#')) {
    throw proxyError('proxy.server must not include a path, query string, or fragment');
  }
