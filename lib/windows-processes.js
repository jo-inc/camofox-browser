...
const WINDOWS_PROCESS_QUERY_ARGS = [
  '-NoLogo', '-NoProfile', '-NonInteractive', '-Command', WINDOWS_PROCESS_QUERY,
];

const WINDOWS_BROWSER_PROCESS_NAMES = new Set([
  'camoufox.exe',
  'camoufox-bin.exe',
  'firefox.exe',
  'firefox-esr.exe',
...
function queryWindowsProcessesAsync() {
  return new Promise((resolve) => {
    cp.execFile(
      ...
      { encoding: 'utf8', windowsHide: true, env: childEnvironment(), timeout: 5000, maxBuffer: 8 * 1024 * 1024 },
...
/** Refresh the non-blocking process snapshot used by periodic resource checks. */
export function refreshWindowsProcesses() {
  if (process.platform !== 'win32') return Promise.resolve([]);
  if (windowsProcessRefresh) return windowsProcessRefresh;

  windowsProcessRefresh = queryWindowsProcessesAsync()
    .then((processes) => {
      windowsProcessCache = processes;
      return processes;
    })
    .finally(() => {
      windowsProcessRefresh = null;
    });
  return windowsProcessRefresh;
}

export function cachedWindowsProcesses() {
  return windowsProcessCache || [];
}

export function isWindowsBrowserProcess(processRecord) {
  const name = String(processRecord?.name ?? processRecord ?? '').trim().toLowerCase();
  return WINDOWS_BROWSER_PROCESS_NAMES.has(name);
}
