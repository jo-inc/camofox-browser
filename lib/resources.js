...
import fs from 'fs';
import { cachedWindowsProcesses, isWindowsBrowserProcess, selectWindowsProcessTree } from './windows-processes.js';
 ...
 * Windows uses the cached working set in MiB; Linux reads RSS from /proc.
 */
export function browserProcessTreeRssMb(browserPid) {
  ...
  if (process.platform === 'win32') {
    const processes = cachedWindowsProcesses();
    if (!processes.some(({ pid }) => pid === browserPid)) return null;
    return windowsWorkingSetMb(selectWindowsProcessTree(browserPid, processes));
  }
...
function windowsWorkingSetMb(processes) {
  if (processes.length === 0 || processes.some((proc) => !Number.isFinite(proc.workingSetSize))) return null;
  return Math.round(processes.reduce((total, proc) => total + proc.workingSetSize, 0) / 1048576);
}
...
export function browserProcessNameRssMb() {
  if (process.platform === 'win32') {
    const processes = selectWindowsProcessTree(process.pid, cachedWindowsProcesses());
    return windowsWorkingSetMb(processes.filter(isWindowsBrowserProcess));
  }
  if (process.platform !== 'linux') return null;
  let totalKb = 0;
  let found = false;
...
export function collectResourceSnapshot(opts = {}) {
  ...
  if (process.platform === 'win32') {
    snap.browserRssMb ??= browserProcessNameRssMb();
    snap.browserMemoryMetric = 'workingSet';
  }
