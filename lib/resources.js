...
import fs from 'fs';
import { cachedWindowsProcesses, selectWindowsProcessTree } from './windows-processes.js';
 ...
 */
export function browserProcessTreeRssMb(browserPid, processSnapshot = null) {
  if (process.platform === 'win32') {
    if (!browserPid || !Number.isInteger(browserPid) || browserPid <= 0) return null;
    const processes = processSnapshot || cachedWindowsProcesses();
    const tree = selectWindowsProcessTree(browserPid, processes);
    if (!tree.some((proc) => proc.pid === browserPid)) return null;
    const totalBytes = tree.reduce((total, proc) => total + proc.workingSetSize, 0);
    return Math.round(totalBytes / 1048576);
  }
 ...
 */
export function browserProcessNameRssMb(processSnapshot = null) {
  if (process.platform === 'win32') {
    const processes = processSnapshot || cachedWindowsProcesses();
    let totalBytes = 0;
    let found = false;
    for (const proc of processes) {
      ...
      if (!/^(?:camoufox(?:-bin)?|firefox(?:-esr)?)(?:\.exe)?$/.test(name)) continue;
      totalBytes += proc.workingSetSize;
      found = true;
    }
    return found ? Math.round(totalBytes / 1048576) : null;
  }
  if (process.platform !== 'linux') return null;
  let totalKb = 0;
  let found = false;
