...
function attachDownloadListener(tabState, tabId, log, pluginEvents) {
  ...
  tabState.page.on('download', async (download) => {
    ...
    if (pluginEvents) {
      pluginEvents.emit('tab:download:start', { userId: null, tabId, filename: suggestedFilename, url });
    }
    ...
    if (pluginEvents && !failure) {
      pluginEvents.emit('tab:download:complete', { userId: null, tabId, filename: suggestedFilename, path: filePath, size: bytes });
    }
