...
// Hard cap on the redirect-status-code history. A non-redirected navigation
// resets it to [], but a SPA that never does a top-level non-redirected
// navigation again would otherwise grow this array without bound for the
// life of the tab.
const MAX_REDIRECT_STATUS_CODES = 20;

/**
 * Create a health tracker for a tab. Attaches to Playwright page events.
 * Always tracks: crashes, page errors, and auto-dismisses dialogs.
 *
 * Request-level tracking (in-flight count, failures, HTTP status histogram,
 * redirect chain, anti-bot challenge detection) is opt-in via
 * `options.trackRequests` and OFF by default.
 *
 * Why: Playwright only releases its Request/Response dispatcher objects
 * when the *page* closes, not when a request finishes -- attaching any of
 * `request`/`response`/`requestfinished`/`requestfailed` for the life of a
 * long-lived tab makes every intercepted request pin a Request+Response
 * pair (plus their internal bookkeeping) in memory until the tab is
 * closed. Measured: a tab with these listeners retained +60.6 MB of shallow
 * heap size (+945k nodes) after 3000 requests and never gave it back after
 * GC; the same tab with only crash/pageerror/dialog retained +0.66 MB
 * (+5.5k nodes) and returned to baseline. See
 * docs/infra/x-signals/preuves/32-instantanes-de-tas.txt and
 * docs/infra/camofox/patches/README.md (0003) for the full measurement.
 * All count-based -- no URLs or content stored, even when enabled.
 */
export function createTabHealthTracker(page, options) {
  const trackRequests = !!(options && options.trackRequests);
  const health = {
    crashes: 0,
    pageErrors: 0,
    requestFailures: 0,
  ...
  // Renderer crash (OOM, segfault) -- always on, load-bearing.
  page.on('crash', () => { health.crashes++; });

  // Uncaught JS exceptions on the page -- always on, load-bearing.
  page.on('pageerror', () => { health.pageErrors++; });

  // Auto-dismiss dialogs to prevent page hangs (not tracked as a metric --
  // noise) -- always on, load-bearing.
    ...
    page.on('response', (resp) => {
      try {
        ...
        if (req.isNavigationRequest()) {
          ...
          // Bounded: a SPA that never does a non-redirected top-level
          // navigation again would otherwise never hit the reset above.
          if (health.redirectStatusCodes.length > MAX_REDIRECT_STATUS_CODES) {
            health.redirectStatusCodes.shift();
          }
