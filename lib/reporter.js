...
function formatIssueBody(type, detail) {
  ...
  if (detail.nativeMemory) {
    ...
    else if (detail.platform && detail.platform !== 'linux') sections.push(`- **browser RSS (last seen):** not available on ${detail.platform} (Linux /proc only)`);
...
export function createReporter(config) {
  ...
  function startWatchdog(thresholdMs = 5000, getContext) {
    ...
    const NATIVE_MEM_LEAK_THRESHOLD_MB = 400; // alert only when growth exceeds self-healing threshold (idle)
    const NATIVE_MEM_ACTIVE_THRESHOLD_MB = 800; // alert when growth exceeds normal active working set (active sessions)
            ...
            if (nativeMemGraceRemaining > 0) {
              ...
              try {
                if (getContext) {
                  ...
                  activeSessions = ctx.sessions ?? ctx.resourceOpts?.sessionCount ?? 0;
                }
              } catch { /* swallow */ }

              // Active sessions legitimately allocate 400-600MB of CDP buffers and DOM structures.
              // Use higher threshold (800MB) when sessions are active to prevent false positives.
              const leakThreshold = activeSessions > 0 ? NATIVE_MEM_ACTIVE_THRESHOLD_MB : NATIVE_MEM_LEAK_THRESHOLD_MB;

              if (growth > leakThreshold && !nativeMemAlertFired) {
