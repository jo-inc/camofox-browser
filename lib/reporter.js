 ...
 * Always tracks crashes, page errors, and dialogs. Request-level tracking is
 * opt-in because long-lived request/response listeners retain Playwright
 * dispatcher objects until the page closes.
 * All count-based -- no URLs or content stored.
 */
export function createTabHealthTracker(page, options = {}) {
  const trackRequests = options?.trackRequests === true;
  const health = {
    crashes: 0,
    pageErrors: 0,
    requestFailures: 0,
...
function formatIssueBody(type, detail) {
  ...
  const browserMemoryLabel = r?.browserMemoryMetric === 'workingSet' ? 'working set' : 'RSS';
  if (r) {
    sections.push('', '## Resources');
    sections.push(`- **node RSS:** ${r.nodeRssMb ?? '?'} MB`);
    sections.push(`- **node heap:** ${r.nodeHeapUsedMb ?? '?'} / ${r.nodeHeapTotalMb ?? '?'} MB`);
    if (r.browserRssMb != null) sections.push(`- **browser ${browserMemoryLabel}:** ${r.browserRssMb} MB`);
  ...
  if (detail.nativeMemory) {
    ...
    if (nm.lastSeenBrowserRssMb != null) sections.push(`- **browser ${browserMemoryLabel} (last seen):** ${nm.lastSeenBrowserRssMb} MB`);
    else sections.push(`- **browser ${browserMemoryLabel} (last seen):** not captured (browser already dead)`);
  }
