...
export function createReporter(config) {
  ...
  function startWatchdog(thresholdMs = 5000, getContext) {
    ...
    const checkMs = 1000;
    // Drifts above this are OS sleep/suspend, not real event-loop stalls.
    // Real stalls are typically 5-60s; 120s+ is always suspend/hibernate.
    const maxReportableDriftMs = 120_000;
    // After a large drift (likely wake from sleep), suppress reports for
    // this many ticks to avoid false positives from post-wake jitter.
    const suppressTicksAfterWake = 5;
    ...
    watchdogInterval = setInterval(() => {
      ...
      // OS sleep/suspend — skip and suppress subsequent jitter reports
      if (drift > maxReportableDriftMs) {
        suppressRemaining = suppressTicksAfterWake;
        return;
      }

      // Post-wake jitter suppression
